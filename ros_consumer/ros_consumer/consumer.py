...
class ROSSkeletonPotentialField(Node):
    def __init__(self):
        ...
        self.sub_skeleton = self.create_subscription(
            SkeletonList, "/skeleton_list", self.skeleton_callback, 10
        )
        ...
        for skeleton in msg.skeletons:
            ...
            for k_id in [ID_KNEE_L, ID_KNEE_R]:
                if k_id in parts:
                    ...
                    v_rep = self.calcular_v_repulsiva(
                        self.pose_robo[0], self.pose_robo[1], ox, oy
                    )
                    v_total += v_rep
    ...
    def render_heatmap(self):
        ...
        if self.obstaculos:
            self.ax.contourf(
                X, Y, Z_pot, zdir="z", offset=-0.05, cmap="Reds", alpha=0.5
            )
